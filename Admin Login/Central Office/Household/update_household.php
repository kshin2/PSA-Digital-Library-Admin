...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['new_pdf']) && $_FILES['new_pdf']['error'] === 0) {
        ...
        $targetDir = "pdfs_household/";
        $targetFilePath = $targetDir . $fileName;

        if (move_uploaded_file($_FILES["new_pdf"]["tmp_name"], $targetFilePath)) {
            // Update title, file, and project
            $stmt = $conn->prepare("UPDATE household SET household_title = ?, household_pdf = ?, household_project = ? WHERE id = ?");
        ...
        $stmt = $conn->prepare("UPDATE household SET household_title = ?, household_project = ? WHERE id = ?");
        $stmt->bind_param("ssi", $title, $project, $id);
    }

    if ($stmt->execute()) {
        echo "Household record updated successfully.";
    } else {
        echo "Failed to update household record.";
    }

...
<meta charset="UTF-8">
    ...
    <link rel="stylesheet" href="householdstyles.css">
    <script src="household-script.js" defer></script>
...
<body>
            ...
            <li class="central-office">
                ...
                <li class="category-item">
                    ...
                    <ul class="subcategory-list">
                        <li><a href="../Household/household_index.php?project=Labor+Force+Survey+(LFS)">Labor Force Survey (LFS)</a></li>
                        <li><a href="../Household/household_index.php?project=Functional+Literacy,+Education+and+Mass+Media+Survey+(FLEMMS)">Functional Literacy, Education and Mass Media Survey (FLEMMS)</a></li>
                        <li><a href="../Household/household_index.php?project=Family+Income+and+Expenditure+Survey+(FIES)">Family Income and Expenditure Survey (FIES)</a></li>
                        <li><a href="../Household/household_index.php?project=Consumer+Expectations+Survey+(CES)">Consumer Expectations Survey (CES)</a></li>
                        <li><a href="../Household/household_index.php?project=National+Information+and+Communications+Technology+Household+Survey+(NICTHS)">National Information and Communications Technology Household Survey (NICTHS)</a></li>
                        <li><a href="../Household/household_index.php?project=Household+Energy+Consumption+Survey+(HECS)">Household Energy Consumption Survey (HECS)</a></li>
                        <li><a href="../Household/household_index.php?project=Household+Survey+on+Domestic+Visitors+(HSDV)">Household Survey on Domestic Visitors (HSDV)</a></li>
                        <li><a href="../Household/household_index.php?project=National+Demographic+and+Health+Survey+(NDHS)">National Demographic and Health Survey (NDHS)</a></li>
                        <li><a href="../Household/household_index.php?project=National+Migration+Survey+(NMS)">National Migration Survey (NMS)</a></li>
                    </ul>
                </li>
  ...
  <section class="record-layout">
    <div class="text"><i class='bx bx-building-house'></i>Household</div>
    ...
    <div class="search-container">
        <div class="left-side">
                ...
                <div id="householdModal" class="modal">
                  <div class="modal-content">
                    ...
                    <div class="input-group">
                        <input class="input_title" type="text" name="household_title" placeholder="Enter Title" required><br><br>

                        <select name="household_project" class="input_project" required>
                        ...
                        <input class="input_file" type="file" name="household_pdf" accept="application/pdf" required><br><br>
            ...
            <div class="right-side">
                ...
                <input type="text" id="searchInput" placeholder="Search for Household Projects..." class="input-search">
...
<script>
 $(document).ready(function() {
  const apiEndpoint = "fetch_household.php";
  const recordsPerPage = 10;
  let currentPage = 1;
  let totalPages = 1;
  let household = [];
  let filteredHousehold = [];
  ...
  function loadHousehold() {
    $.ajax({
      ...
      success: function(data) {
        ...
        if (data.length === 0) {
          $('#recordsTable tbody').append('<tr><td colspan="6">No household records found.</td></tr>');
          return;
        }

        household = data;
        filteredHousehold = [...household];

        updatePagination();
        loadTablePage(currentPage);
      },
      error: function() {
        $('#status').text("Failed to load household data.");
      }
    });
  }
  ...
  function loadTablePage(page) {
    ...
    let paginatedData = filteredHousehold.slice(start, end);
    ...
    tbody.fadeOut(200, function () {
      ...
      paginatedData.forEach(entry => {
        tbody.append(`
          <tr class="total-household">
              <td>${entry.household_title}</td>
              <td>${entry.household_project}</td>
              <td><a href='pdfs_household/${encodeURIComponent(entry.household_pdf)}' target='_blank'>View PDF</a></td>
              <td>
              <div class="modify-container">
                  ...
                  <button class="edit-btn"
                    data-id="${entry.id}"
                    data-title="${entry.household_title}"
                    data-project="${entry.household_project}"
                    data-file="${entry.household_pdf}">
  ...
  if (projectParam) {
    // Decode and set the filter dropdown (if you have one)
  ...
  // Dropdown filter change (optional)
...
function applyProjectFilter(project) {
  if (project && project !== "all") {
    filteredHousehold = household.filter(entry => entry.household_project === project);
  } else {
    filteredHousehold = [...household];
  }
  ...
  function updatePagination() {
    totalPages = Math.ceil(filteredHousehold.length / recordsPerPage);
  ...
  function searchHousehold() {
    ...
    filteredHousehold = household.filter(entry =>
    entry.household_title.toLowerCase().includes(searchTerm) ||
    entry.household_pdf.toLowerCase().includes(searchTerm) ||
    entry.household_project.toLowerCase().includes(searchTerm)
    );
  ...
  $(document).on("click", ".edit-btn", function() {
    ...
    $("#currentFileLink").attr("href", `pdfs_household/${file}`);
  ...
  $("#confirmDeleteBtn").click(function() {
    $.post("delete_household.php", { id: deleteId }, function(response) {
      alert(response);
      $("#deleteModal").fadeOut();
      loadHousehold();
    });

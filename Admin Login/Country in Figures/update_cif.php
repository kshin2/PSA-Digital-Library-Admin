...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['new_pdf']) && $_FILES['new_pdf']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES["new_pdf"]["tmp_name"], $targetFilePath)) {
            ...
            $stmt = $conn->prepare("UPDATE cif SET title_cif = ?, pdf_cif = ? WHERE id = ?");
        ...
        $stmt = $conn->prepare("UPDATE cif SET title_cif = ? WHERE id = ?");
        $stmt->bind_param("si", $title, $id);
    }

    if ($stmt->execute()) {
        echo "CIF record updated successfully.";
    } else {
        echo "Failed to update CIF record.";
    }

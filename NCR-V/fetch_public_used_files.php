...
$searchQuery = $queryStr ? "WHERE title_puf LIKE '%$queryStr%'" : "";

// Get total filtered records
$totalResult = $conn->query("SELECT COUNT(*) AS total FROM puf $searchQuery");
$totalRow = $totalResult->fetch_assoc();
$totalRecords = $totalRow['total'];
$totalPages = ceil($totalRecords / $limit);

// Get paginated filtered records
$query = $conn->query("SELECT * FROM puf $searchQuery ORDER BY created_at DESC LIMIT $start, $limit");

$output = '';
if ($query->num_rows > 0) {
    while ($row = $query->fetch_assoc()) {
        $output .= '<tr>
            <td>' . htmlspecialchars($row['title_puf']) . '</td>
            <td class="pdf-cell">';
        if (!empty($row['pdf_puf'])) {
            $output .= '<a href="' . htmlspecialchars($row['pdf_puf']) . '" target="_blank">
